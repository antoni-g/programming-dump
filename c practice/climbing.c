...
int* climbingLeaderboard(int scores_count, int* scores, int alice_count, int* alice, int* result_count) {
    int* ret = malloc(alice_count*sizeof(int));
    *result_count = alice_count;
    
    int currRank = getLastRank(scores,scores_count)+1;
    int currIndex = scores_count-1;
    
    for (int i = 0; i < alice_count; i++) {
        while(true) {
            if (scores[currIndex] > alice[i]) {
                ret[i] = currRank;
                break;
            }
            else {
                if (currRank == 1) {
                    if (alice[i] < scores[currIndex]) {
                        ret[i] = 2;
                    }
                    else {
                        ret[i] = 1;
                    ...
                    if (scores[currIndex] < scores[currIndex-1]) {
                        currRank--;
                    }
                    currIndex--;
                }
...
int getLastRank(int* scores, int scores_count) {
    int rank = 1;
